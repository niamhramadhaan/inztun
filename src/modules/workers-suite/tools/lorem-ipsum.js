...
export class LoremIpsum {
  ...
  init(root) {
    this.typeEl = root.querySelector('#li-type');
    this.countEl = root.querySelector('#li-count');
    this.outputEl = root.querySelector('#li-output');
    this.countDisplayEl = root.querySelector('#li-output-count');
  ...
  paragraph() { return Array.from({ length: Math.floor(Math.random() * 4) + 3 }, () => this.sentence()).join(' '); }
  sentence() {
    const words = Array.from({ length: Math.floor(Math.random() * 8) + 5 }, () => this.word());
    words[0] = words[0].charAt(0).toUpperCase() + words[0].slice(1);
    return words.join(' ') + '.';
  }
  word() { return this.words[Math.floor(Math.random() * this.words.length)]; }
