...
export class JsonFormatter {
  ...
  init(root) {
    this.inputEl = root.querySelector('#jf-input');
    this.outputEl = root.querySelector('#jf-output');
    this.countEl = root.querySelector('#jf-input-count');
  ...
  validate() {
    try {
      ...
      this.outputEl.textContent = `✗ Invalid: ${e.message}`;
      this.outputEl.style.color = 'var(--color-error)';
      Toast.error('Invalid JSON');
    }
  }
